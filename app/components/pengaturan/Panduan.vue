	...
	const langkah: PanduanLangkah[] = [
		...
		},
		{
			...
			isi: [
				...
				"Mau lihat siapa saja yang sedang online (login tapi belum tentu lagi buka soal)? Buka menu Siswa — di pojok kanan atas ada badge \"X online\" plus tabelnya, update sendiri tiap 3 detik.",
				"Mau lihat progres pengerjaan soal secara live (siapa yang sudah mulai, baru sampai soal berapa, sudah selesai)? Buka Hasil Ujian, pilih Kelas, pilih Mata Pelajaran, pilih Jenis Ujian, lalu buka tab Daftar Siswa — halaman ini juga update sendiri tiap 3 detik tanpa perlu di-refresh."
			]
		},
		{
			nomor: 7,
			...
			menu: "Hasil Ujian",
			isi: [
				"Pilih kelas, pilih mata pelajaran, lalu pilih jenis ujiannya (satu jenis ujian = satu paket hasil terpisah, nggak kecampur sama jenis ujian lain di mapel yang sama). Tab Daftar Siswa menampilkan progres dan nilai tiap siswa (nilai yang muncul di sini cuma dari soal pilihan ganda — otomatis dihitung sistem).",
				"Kalau ada soal esai, klik Detail di nama siswa untuk buka halaman penilaiannya. Baca jawaban esainya, isi nilai per soal, klik Simpan Nilai. Nilai esai ini sengaja dipisah dan nggak otomatis digabung ke nilai pilihan ganda — guru yang putuskan sendiri nilai akhirnya.",
				"Tab Analisis Soal menunjukkan soal mana yang paling banyak dijawab salah dan siapa yang nilainya paling tinggi/rendah — enak buat lihat pola cepat tanpa ngecek satu-satu.",
				"Butuh rekapnya di luar aplikasi? Klik Export (bisa pilih Semua, per Siswa, atau Bulanan) untuk unduh nilai sebagai file Excel."
			]
		}
	];
	...
	<div class="space-y-6 w-full">
		<UCard class="bg-primary/5 border-primary/20">
			<div class="flex items-start gap-3">
				<UIcon name="lucide:map" class="size-6 text-primary shrink-0 mt-0.5" />
				<div>
					<p class="font-semibold">
						Dari nol sampai lihat hasil ujian
					</p>
					<p class="text-sm text-muted mt-1">
						Ikuti urutannya dari atas ke bawah — tiap langkah butuh langkah sebelumnya selesai
						dulu. Anggap saja ini seperti nyiapin kelas baru di awal semester: kenalan dulu sama
						aplikasinya, masukin data anak-anak, siapin soal, baru buka ujiannya.
					</p>
				</div>
			</div>
		</UCard>

		<UCard v-for="l in langkah" :key="l.nomor">
			<template #header>
		...
		<UCard variant="subtle">
			<div class="flex items-center gap-3">
				<UIcon name="lucide:layout-grid" class="size-5 text-muted shrink-0" />
				<p class="text-sm text-muted">
					Mau tahu fitur tambahan seperti Import dari Word, Tarik Soal Online, Tarik Data Siswa, dan Export Bank
					Soal? Lihat di <NuxtLink to="/tentang/fitur" class="text-primary hover:underline">Tentang > Fitur</NuxtLink>.
				</p>
			</div>
		</UCard>
